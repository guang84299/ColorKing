...
GRockView* GRockView::create(GRockCallback callback)
{
    GRockView* view = new GRockView();
    if(view && view->init(callback))
    {
        view->autorelease();
        return view;
    }
...
//    _bg->setScale(r / bg_w, r / bg_w);
    ...
    isUpdate = false;
    rdt = 0;
    this->schedule(SEL_SCHEDULE(&GRockView::update));
    hide();
    return true;
}

bool GRockView::init(GRockCallback callback)
{
    auto listener = EventListenerTouchOneByOne::create();
    listener->onTouchBegan = CC_CALLBACK_2(GRockView::onTouchBegan, this);
//    listener->onTouchMoved = CC_CALLBACK_2(GRockView::onTouchMoved, this);
...
bool GRockView::onTouchBegan(Touch* touch, Event  *event)
{
    ...
    _callback(location,1,"move");
    
    auto sp = Sprite::create("pos.png");
    sp->setScale(0.1f);
    sp->setPosition(location);
    sp->runAction(Sequence::create(EaseSineOut::create(ScaleTo::create(0.5f, 0.5f)),
                                   FadeOut::create(0.2f),
                                   RemoveSelf::create(), NULL));
    this->addChild(sp);
    
    sp = Sprite::create("pos.png");
    sp->setScale(0.1f);
    sp->setOpacity(0);
    sp->setPosition(location);
    sp->runAction(Sequence::create(FadeIn::create(0.2f),
                                   EaseSineOut::create(ScaleTo::create(0.5f, 0.5f)),
                                   FadeOut::create(0.2f),
                                   RemoveSelf::create(), NULL));
    this->addChild(sp);
    ...
    sp->setOpacity(0);
    sp->setPosition(location);
    sp->runAction(Sequence::create(FadeIn::create(0.4f),
                                   EaseSineOut::create(ScaleTo::create(0.5f, 0.5f)),
                                   FadeOut::create(0.2f),
                                   RemoveSelf::create(), NULL));
    this->addChild(sp);
